...
const getContentMarkdown = async (directory: string, id: string) => {
    ...
    return {
        ...
        title: matterResult.data.title,
        description: matterResult.data.description,
        date: moment(matterResult.data.date, "DD-MM-YYYY").format("dddd, MMMM Do YYYY"),
        authors: Array.isArray(matterResult.data.authors) ? matterResult.data.authors : [],
        keywords: Array.isArray(matterResult.data.keywords) ? matterResult.data.keywords : []
    }
}
